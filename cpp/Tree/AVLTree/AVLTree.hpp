...
public:
    ...
    /** 关于移动构造函数的一点小总结:
     *  经过测试发现了以下规律:
     *  1.如果当前类定义了拷贝构造函数, 而没有定义移动构造函数,则使用move()调用对象的移动构造函数时,
     *    实际上调用的是拷贝构造函数, 即使当前类的父类定义了移动构造函数, 也不会调用父类的移动构造函数.
     *  2.如果当前类定义了移动构造函数,而没有定义拷贝构造函数, 则程序中需要调用拷贝构造函数的地方会编译不通过,
     *    即使父类已经定义了拷贝构造函数, 也不会调用父类的拷贝构造函数.
     *  3.如果当前类既没有定义拷贝构造函数, 也没有定义移动构造函数, 则代码中使用move()调用移动构造函数时,能否
     *    调用到移动构造函数取决于其直接父类中有没有定义移动构造函数,如果有定义, 则会调用父类的移动构造函数.
     *    如果父类中没有定义移动构造函数,即使祖先类中定义了移动构造函数, 也不会得到调用.
     *   
     *  根据以上规律, 可得到如下总结:
     *  1. 不能在一个类中只定义移动构造函数而不定义拷贝构造函数, 但只定义拷贝构造函数而不定义移动构造函数是可以的.
     *  2. 由于子类不定义拷贝构造函数时,可以调用到父类的移动构造函数, 因此如果子类中没有增加新的成员变量时,也就是
     *     不需要编写自己的拷贝构造函数时, 就最好不要写拷贝构造函数; 如果子类中增加了新的成员变量, 就需要编写自己
     *     的拷贝构造函数, 这时如果复制一个对象的工作量比较大, 最好还编写一个移动构造函数.
     */

    //这里由于没有写拷贝构造函数, 因此也不能写移动构造函数, 这里可以使用父类的移动构造函数
    // AVLTree(AVLTree<T>&& avl)
    // {
    //     this->_root = avl._root;
    //     this->_size = avl._size;
    //     this->_hot = avl._hot;
    //     avl._root = NULL;
    //     avl._hot = NULL;
    //     avl._size = 0;
